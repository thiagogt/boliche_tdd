 ...
 * Pontuacao
 * 
 * Essa classe esta no momento perfeito para uma refatoracao, dado que tem todos
 * os testes passando. Montar um buffer de soma eh muito bem vindo!!!
 */
    ...
    public int calculaSomaPontos() {
        ...
        for (int i = 0; i < NUM_MAX_RODADAS; i++) {
            ...
            for (int j = 0; j < rodada.length; j++) {
                ...
                if (pontoDescrito.equals("X")) {
                    pontoDescrito = "10";
                    strike = (i != 9);
                    duplicadoPorBonusSequencial = pontoAnterior.equals("10");
                }
                if (pontoDescrito.equals("/")) {
                    spare = true;
                    pontoDescrito = "0";
                    duplicadoPorBonusSequencial = pontoAnterior.equals("10");
                }
                ...
                if (spare)
                    try {
                        ...
                        throw new RuntimeException(
                                "Ponto Spare invalido! Nao existe spare na primeira jogada. Não seria Strike? (X):  Rodada: "
                                        + i);
                ...
                // Esse bonus se da quando tem Spare/Strike ou Strike/Strike em seguida. Faz com
                // que o proximo
                // valor seja multiplicado 2x.
                // Nao contar bonus acumulado quando estiver na ultima jogada e nas ultimas 2
                // casas.
                // Aqui pode ser que na 9 jogada final tenha feito um strike e na primeira da 10
                // tb e esse
                // bonus deve contar.
                if (duplicadoPorBonusSequencial && i != 9 && j != 1 && j != 2) {
                    ...
                    if (pontoDescrito.equals("X"))
                        proximoPontoBonus = 10;

                    if (pontoDescrito.equals("-"))
                        proximoPontoBonus = 10;
                    else {
                        proximoPontoBonus = Integer.parseInt(proximoPonto);
                    }
                ...
                if (i == (NUM_MAX_RODADAS - 1)) {
                    if (rodada.length > NUM_MAX_JOGADAS_ULTIMA_RODADA)
                        throw new RuntimeException(
                                "Total de " + rodada.length + " quadros incompativel para ultima rodada");
                    switch (j) {
                    case 0:
                    ...
                    case 1:
                        ...
                        if (primeiroPontoFinal != 10) {
                            if (pontosPorRodada > NUM_MAX_RODADAS && segundoPontoFinal != PINOS_MAX_RODADA)
                                throw new RuntimeException("Somatoria de pontos por rodada invalida: " + pontosPorRodada
                                        + " Rodada: " + i + " na seguda jogada");
                            if (segundoPontoFinal != PINOS_MAX_RODADA && rodada.length > NUM_MAX_JOGAADAS
                                    && !rodada[1].equals("/"))
                                throw new RuntimeException(
                                        "Total de " + rodada.length + " quadros incompativel para rodada: " + i);
                        }
                        break;
                    case 2:
                        ...
                        if (primeiroPontoFinal == 10)
                            if (segundoPontoFinal + terceiroPontoFinal > PINOS_MAX_RODADA && segundoPontoFinal != 10)
                                throw new RuntimeException("Somatoria de pontos por rodada invalida: "
                                        + (segundoPontoFinal + terceiroPontoFinal) + " Rodada: " + i
                                        + " entre a segunda e terceira jogada");
                        break;
                    }
                } else {
                    if (pontosPorRodada > PINOS_MAX_RODADA)
                        throw new RuntimeException(
                                "Somatoria de pontos por rodada invalida: " + pontosPorRodada + " Rodada: " + i);
